...
#if (( "$SERVER_VERSION" == "14" )); then mv /usr/local/bin/kubectl1.14 /usr/local/bin/kubectl; fi 2>/dev/null
#if (( "$SERVER_VERSION" >= "15" )); then mv /usr/local/bin/kubectl1.15 /usr/local/bin/kubectl; fi 2>/dev/null
